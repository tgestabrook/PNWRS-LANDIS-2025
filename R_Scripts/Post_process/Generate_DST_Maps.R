...
if(F %in% file.exists(file.path(landisOutputDir, "DST", paste0(names(new.outputs), ".tif")))){
  ...
  harvestCost.r <- ifel(harvestedMG.r == 0|is.na(harvestedMG.r), 0, harvestCost.r)
  
  ### rxCost.r
  rxCost.r <- RxFireHa.r * ifel(lua.r == 12, RxFire.Wildlands.cost.USD.per.Ha, RxFire.ActiveForest.cost.USD.per.Ha)   # different cost for wildland and managed
...
if(F %in% file.exists(file.path(landisOutputDir, "DST", paste0(names(new.outputs), ".tif")))){
  ...
  ### mort.MG.r 
  # mort.Mg.r <- classify(fireIdStack.r, rcl=data.frame('is'=fire.df$EventID, 'becomes'=fire.df$TotalBiomassMortality / fire.df$TotalSitesBurned)) * 0.01 * 0.81
  join.df <- fire.df |>
  ...
  ### areaBurnedNonWilderness.Ha.r
  areaBurnedNonWilderness.Ha.r <- ifel(!is.na(severityStackSmoothedClassified.r) & lua.r != 12, 0.81, 0)
...
### SMOKE: ---------------------------------------------------------------------
new.outputs<-c('emissions.pm2.5.Kg.r','emissions.pm10.Kg.r'
               #, 'emissions.timing.r'
)
names(new.outputs)<-c('Smoke_Emissions_Kg_pm25','Smoke_Emissions_Kg_pm10'
                      #, 'Smoke_Timing_quantile'
)
outputs <- c(outputs, new.outputs)
...
if(F %in% file.exists(file.path(landisOutputDir, "DST", paste0(names(new.outputs), ".tif")))){
  ...
  #   cat(paste0('\n...', yr))
  #   focal.yr.fire.df <- fire.df |> filter(SimulationYear == yr)
  ...
  #     
  #   } else emissions.timing.r <- zero.r[[1]]
  #   
...
new.outputs<-c('age.gt.120.r','seed.source.r',
               'seed.source.dist.r','delta.offsite.biomass.r'
               # ,'patches.r'
               )
names(new.outputs)<-c('Old_Forest','Seed_Source',
                      'Seed_Source_Distance_m','Delta_Offsite_Biomass_Mg'
                      # ,'Area_of_Homogeneous_Forest_ha'
                      )
outputs <- c(outputs, new.outputs)


if(F %in% file.exists(file.path(landisOutputDir, "DST", paste0(names(new.outputs), ".tif")))){
  mean.age.top3.dom.r <- rast(file.path(ageOutput, "MeanAge_TopThreeSpecies-yr.tif"))
  ...
  seed.source.r <- ifel(mean.age.top3.dom.r < 20|is.na(mean.age.top3.dom.r), 0, 1)
  ...
  focal.yr.dominant.spp <- rast(file.path(ageOutput, "DominantSpecies-yr.tif"))
  focal.yr.dominant.spp2 <- rast(file.path(ageOutput, "DominantSpeciesTwo-yr.tif"))
  
  spp <- BiomassTrees.stack |> select(contains("-0")) |> names() |> str_remove("-0-biomass")
  ...
  delta.1yr.biomass.r <- c(zero.r[[1]], totalBiomass_stack.r[[2:simLength]] -  totalBiomass_stack.r[[1:(simLength - 1)]])  # get the 1-year change at each timestep
  ...
  age.class.r<-round(mean.age.top3.dom.r/40, 0) * 40
  
  age.classes<-unique(as.vector(values(age.class.r)))
  cover.types<-unique(as.vector(values(cover.type.r)))
  
  # gc()
  # cat('  - Area of homogenous patches... ')
  # 
  # area.r <- rast(zero.r[[1]], vals = 0.81)
  # 
  # gc()
  # patches.r <- foreach(yr = yrs) %do% {
  #   r <- patches(age.class.r[[yr+1]] + (cover.type.r[[yr+1]]/2), values = T, zeroAsNA = T) |> zonal(area.r, fun = "sum", as.raster = T)  # this should work to give each age x cover combo a unique value
  # }
  
  # test <- patches(age.class.r, values = T, zeroAsNA = T)
  # 
  # patches.r<-zero.r
  ...
  #     r <- ifel(age.class.r == a & cover.type.r == c, 1, 0) |> patches() |> zonal(rast(zero.r, vals = 0.81), fun = "sum", as.raster = T)
  # 
  #     r <- ifel(is.na(r), 0, r)
  #     patches.r<-patches.r+r
  #     
  #     
  #     # Calculate area
  ...
  cover.type2.r <- ifel(mean.age.top3.dom.r < 20, 0, cover.type.r) ## Set stand initation phase to 0
...
if(
  ...
  for (i in 1:length(outputs)){
    cat(outputs[i])
    out.name <- names(outputs[i])

    r <- rast(file.path(landisOutputDir,"DST",paste0(out.name,'.tif')))
...
# test <- raster2csv(delta.biomass.r, pwg.r)

# test <- zonal(delta.biomass.r, pwg.r, fun = sd, wide = F, na.rm = T)
# 
# test <- zonal(delta.biomass.r, pwg.r, fun = sd, wide = T, na.rm = T) |> pivot_longer(names(delta.biomass.r), names_to = "layer", values_to = "value") |> mutate(metric = "sd")
