...
```{r, fig.cap = "Another fire size comparison"}
df <- bind_rows(
  fire.df |> select(FIRE_SIZE) |> mutate(Source = "LANDIS"),
  mtbs.df |> select(FIRE_SIZE) |> mutate(Source = "MTBS")
)

ggplot(data = df, aes(x = log10(FIRE_SIZE), fill = Source, color = Source)) + geom_density(alpha = 0.25) + ggtitle("Event Size Distribution") +
