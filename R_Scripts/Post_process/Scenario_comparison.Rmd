...
ggplot(data = df, aes(x = SimulationYear, y = Mean, color = Scenario)) +
  geom_line() +
  geom_ribbon(aes(ymin=Min,ymax=Max),alpha=0.2, linewidth = NA) +
  scale_color_manual(values = scenario_colors) + scale_fill_manual(scenario_colors) +
...
```{r, fig.cap="Wildfire area burned per severity class (deviation from reference scenario)"}
ggplot(data = df, aes(x = SimulationYear, y = Mean, color = Scenario2)) +
  geom_line() +
  geom_ribbon(aes(ymin=Min,ymax=Max),alpha=0.2, linewidth = NA) +
...
```{r, fig.cap="Area burned per severity class (deviation from reference scenario)"}
df <- fires.all.df |>
  group_by(Run, SimulationYear, Scenario, Scenario2, Severity) |>
  summarise(Area_total = sum(Area_ha)) |>  # total annual area in each severity class
  ungroup() |>
...
ggplot(data = df, aes(x = Year, fill = Scenario, color = Scenario)) + 
...
```{r, fig.cap="Total harvested biomass by PWG"}
if(nrow(df) > 0){
  ggplot(data = df, aes(x = Year, fill = Scenario2, color = Scenario2)) + 
    geom_line(aes(y = Mean)) +
    geom_ribbon(aes(ymin=Min,ymax=Max),colour=NA,alpha=0.2,size=0.25) +
