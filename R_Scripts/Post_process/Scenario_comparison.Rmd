...
ggplot(data = df, aes(x = Year, fill = Scenario, color = Scenario, fill = Scenario)) + 
    ...
    xlab("Simulation Year") + ylab(expression(paste('Peak SWE (m)')))

```

```{r, fig.cap="Mean Annual Flow"}
df <- dst.huc12.all.df |>
  filter(Metric == "MeanAnnualFlow", Year > 0) |>
  group_by(Year, HUC12.num, Scenario, Scenario2) |>
  summarise(
    Mean = mean(mean),
    Max = max(mean),
...
ggplot(data = df, aes(x = Year, fill = Scenario, color = Scenario, fill = Scenario)) + 
    ...
    scale_fill_manual(values = scenario_colors) + scale_color_manual(values = scenario_colors) +
    facet_grid(Scenario2~.) +
    xlab("Simulation Year") + ylab(expression(paste('MeanAnnualFlow (m)')))
```

```{r, fig.cap="Merchantable Biomass"}
df <- dst.huc12.all.df |>
  filter(Metric == "Harvested_Merch_Mg") |>
...
ggplot(data = df, aes(x = Year, fill = Scenario, color = Scenario, fill = Scenario)) + 
    ...
    scale_fill_manual(values = scenario_colors) + scale_color_manual(values = scenario_colors) +
    facet_grid(Scenario2~.) +
    xlab("Simulation Year") + ylab(expression(paste('Harvested Merchantable Biomass (Mg)')))
```

```{r, fig.cap="Net revenue (harvest revenue - management cost)"}
df <- dst.huc12.all.df |>
  filter(Metric %in% c("Revenue_Merch_USD", "Revenue_Chip_USD", "Harvest_Cost_USD", "Rx_Fire_Cost_USD", "Fire_Suppression_Cost_USD")) |>
  select(Year, HUC12.num, Scenario, Scenario2, Run, Metric, sum) |>
  mutate(sum = replace_na(sum, 0)) |>
  pivot_wider(names_from = "Metric", values_from = "sum") |>
  mutate(NetRevenue = Revenue_Merch_USD + Revenue_Chip_USD - Harvest_Cost_USD - Rx_Fire_Cost_USD - Fire_Suppression_Cost_USD) |>
  group_by(HUC12.num, Run) |>
  arrange(Year) |> mutate(NetRevenue = cumsum(NetRevenue)) |>
  ungroup() |>
  group_by(Year, HUC12.num, Scenario, Scenario2) |>
  summarise(
    Mean = mean(NetRevenue, na.rm = T)/1000000,
    Max = max(NetRevenue, na.rm = T)/1000000,
    Min = min(NetRevenue, na.rm = T)/1000000
  ) |> ungroup() |># summarise mean of all HUC12
...
```{r, fig.cap="Area burned in WUI"}
df <- dst.huc12.all.df |>
  filter(Metric == "Fire_Area_Burned_nonWilderness_Ha") |>
  mutate(sum = replace_na(sum, 0)) |>
  group_by(HUC12.num, Run) |>
  arrange(Year) |>
  mutate(
...
ggplot(data = df, aes(x = Year, fill = Scenario, color = Scenario, fill = Scenario)) + 
    ...
    scale_fill_manual(values = scenario_colors) + scale_color_manual(values = scenario_colors) +
    facet_grid(Scenario2~.) +
    xlab("Simulation Year") + ylab(expression(paste('Cumulative area burned in WUI')))
```

```{r, fig.cap="Mature forest area"}
df <- dst.huc12.all.df |>
  filter(Metric == "Old_Forest") |>
  mutate(sum = replace_na(sum, 0)) |>
  group_by(Year, HUC12.num, Scenario, Scenario2) |>
  summarise(
    Mean = mean(sum, na.rm = T),
...
ggplot(data = df, aes(x = Year, fill = Scenario, color = Scenario, fill = Scenario)) + 
    ...
    scale_fill_manual(values = scenario_colors) + scale_color_manual(values = scenario_colors) +
    facet_grid(Scenario2~.) +
    xlab("Simulation Year") + ylab(expression(paste('Area of Mature Forest (Ha)')))
```
...
```{r, fig.cap="Smoke emissions"}
df <- dst.huc12.all.df |>
  filter(Metric == "Smoke_Emissions_Kg_pm25") |>
  mutate(sum = replace_na(sum, 0)) |>
  group_by(Year, HUC12.num, Scenario, Scenario2) |>
  summarise(
    Mean = mean(sum, na.rm = T),
...
ggplot(data = df, aes(x = Year, fill = Scenario, color = Scenario, fill = Scenario)) + 
    ...
    scale_fill_manual(values = scenario_colors) + scale_color_manual(values = scenario_colors) +
    facet_grid(Scenario2~.) +
    xlab("Simulation Year") + ylab(expression(paste('Pm 2.5 smoke emissions (kg)')))
```

```{r, fig.cap="Drought-susceptible biomass"}
df <- dst.huc12.all.df |>
  filter(Metric == "Delta_Offsite_Biomass_Mg") |>
  group_by(Year, HUC12.num, Scenario, Scenario2) |>
  summarise(
    Mean = mean(sum, na.rm = T),
    Max = max(sum, na.rm = T),
...
ggplot(data = df, aes(x = Year, fill = Scenario, color = Scenario, fill = Scenario)) + 
    ...
    scale_fill_manual(values = scenario_colors) + scale_color_manual(values = scenario_colors) +
    facet_grid(Scenario2~.) +
    xlab("Simulation Year") + ylab(expression(paste('Drought susceptible biomass (Mg)')))
```
