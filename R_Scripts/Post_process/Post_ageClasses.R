...
if (!file.exists(file.path(ageOutput, 'MeanAge_AllSpp-yr.tif'))){
  ...
  writeRaster(MeanAge.stack,file.path(ageOutput, 'MaxAge_AllSpp-yr.tif'),overwrite=T, datatype = "INT2S")
}
  ...
  foreach (yr = unique(yrs),  .packages = c("terra", "tidyverse", "tidyterra"), .export = c("get_ages_of_top3_biomass"), .verbose = T, .inorder=F, .final = function(x) NULL) %dopar% {
    if(
      ...
      if (!file.exists(file.path(ageOutput, 'MeanAge_DominantSpecies-yr.tif'))){
        writeRaster(mean.age.domSpp.r,file.path(ageOutput, paste0('MeanAge_DominantSpecies-',yr,'.tif')),overwrite=T, datatype = "INT2S")
      } 
      if (!file.exists(file.path(ageOutput, 'MeanAge_TopThreeSpecies-yr.tif'))) {
        writeRaster(mean.age.top3.dom.r,file.path(ageOutput, paste0('MeanAge_TopThreeSpecies-',yr,'.tif')),overwrite=T, datatype = "INT2S")
      } 
      if (!file.exists(file.path(ageOutput, 'DominantSpecies-yr.tif'))) {
        writeRaster(top3sp.r[[1]],file.path(ageOutput, paste0('DominantSpecies-',yr,'.tif')),overwrite=T, datatype = "INT1S")
      } 
      if (!file.exists(file.path(ageOutput, 'DominantSpeciesTwo-yr.tif'))) {
        writeRaster(top3sp.r[[2]],file.path(ageOutput, paste0('DominantSpeciesTwo-',yr,'.tif')),overwrite=T, datatype = "INT1S")
      } 
      if (!file.exists(file.path(ageOutput, 'DominantSpeciesThree-yr.tif'))) {
        writeRaster(top3sp.r[[3]],file.path(ageOutput, paste0('DominantSpeciesThree-',yr,'.tif')),overwrite=T, datatype = "INT1S")
      }
  
      rm(list = c("mean.age.domSpp.r", "mean.age.top3.dom.r", "age.top.3.dom.r", "biomass.trees", "med.age", "top3sp.r"))
      gc()
    }
  }
...
cat(paste0('\nCreating stacks...', 'ageOutput', '...'))

### Remove tif.aux.xml
auxxml <- files <- list.files(path = file.path(landisOutputDir,'ageOutput'), pattern = "\\.aux.xml$", full.names = TRUE)
file.remove(auxxml)

files<-dir(file.path(landisOutputDir,'ageOutput'))
...
for(mapType in mapTypes){
  ...
  if (file.exists(file.path(landisOutputDir, 'ageOutput', outName))){next}
  
  cat(paste0(mapType, '...'))
  oldMaps <- dir(file.path(landisOutputDir, 'ageOutput'))[grepl(paste0("^", mapType), dir(file.path(landisOutputDir, 'ageOutput')))]  # get the names of the maps to delete after loading
  stack <- get_maps('ageOutput', mapType, crs(ecos.r), ext(ecos.r))
  
  writeRaster(stack, file.path(landisOutputDir, 'ageOutput', outName), datatype = "INT2S")
  file.remove(file.path(landisOutputDir, 'ageOutput', oldMaps))  # remove the old loose map files
}
